...
class ContestHelper(cf_base_class.CFBaseClass):
    ...
    def get_contests(self, phase=None):
        ...
        contests = self.cf_api.contest_list()
        if not phase:
            return contests
    ...
    def get_code(self, submission):
        try:
            return self.cf_parser.get_solution(contest_id=submission.problem.contest_id,
    ...
    def get_wa_ac_pairs_for_contest(self, contest_id):
        ...
        all_submissions = self.get_submissions(contest_id)
        for submission in all_submissions:
            print(submission)
            try:
                solution = self.cf_parser.get_solution(submission.contest_id, submission.id)
                print(solution)
...
if __name__ == '__main__':
    ...
    print(finished_contests)
    # pairs = ch.get_wa_ac_pairs_for_contest(1506)
    # print(pairs)
